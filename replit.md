...
- `server/templates/shelter-map.html` - Leaflet map page for shelter finder (legacy, maps now use inline srcdoc)
- `server/templates/disaster-map.html` - Leaflet map page for disaster reports (legacy, maps now use inline srcdoc)
...
## Maps Architecture
- Maps use `srcdoc` attribute on iframes (HTML embedded directly) instead of loading from backend URLs
- This avoids cross-origin/proxy issues when running through Replit's domain proxy
- Map HTML is generated by helper functions: `getDisasterMapHtml()` and `getShelterMapHtml()`
